...
if(length(varargin)>1) 
    data.iso=double(varargin{2});
else
    data.iso=(2/4)*(data.vmax-data.vmin)+data.vmin; % initial ISO level
end
...
data.posy=0.5;  
data.posz=0.5; 
data.scales=[1 1 0.85];
% data.color=[1 0.75 0.65]; %tissue
% data.color=[0.745098 0.745098 0.745098]; %gray
data.color=[0.827451 0.827451 0.827451]; %gray
...
j(j<1)=1;
j(j>size(data.histimage,2))= size(data.histimage,2);
...
az = 154; el = 12;
view(az, el); axis equal; daspect(data.scales); axis off; 
setMyData(data);
...
if(~data.slicerender) 
    ...
    az = 42;
    el = 36;
    view(az, el); axis square; axis off; 
end
...
defaultoptions=struct('V',[0 0 1],'L',[0.67 0.33 0.67],'ka',0.3,'kd',0.5,'ks',0.5,'n',9,'color',[1 1 1],'S',[]);
...
% --- Executes on button press in radiobutton025.
function radiobutton025_Callback(hObject, eventdata, handles)
% hObject    handle to radiobutton025 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of radiobutton025
